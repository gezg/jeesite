...
<html>
<head>
	<title><sitemesh:title default="欢迎光临"/> - ${site.title} - Powered By 戈志刚</title>
	<%@include file="/WEB-INF/views/modules/cms/front/include/head.jsp" %>
	<sitemesh:head/>
</head>
<body>
	<c:choose>
		<c:when test="${cookie.theme.value == 'justice'}">
			<div id="header">
				 <div class="container">
				 	<div class="header-top">
		                <p class="welcome">
		                    <script src="${ctxStaticTheme}/date.js" type="text/javascript"></script>
		                </p>
		                <ul class="top-right">
		                    <li><a href="#" onlick="SetHome(this,location.href)">设为首页</a></li>
		                    <li class="last"><a href="#" onclick="AddFavorite(location.href,'司法网');">加入收藏</a></li>
		                </ul>
		            </div>
		            <ul class="header-nav">
		                ...
		                <li class="header-item ${not empty isIndex && isIndex ? 'active' : ''}"><a href="${ctx}/index-1${fns:getUrlSuffix()}"><span>${site.id eq '1'?'首　 页':'返回主站'}</span></a></li>
						<c:forEach items="${fnc:getMainNavList(site.id)}" var="category" varStatus="status"><c:if test="${status.index lt 6}">
		                    <c:set var="menuCategoryId" value=",${category.id},"/>
				    		<li class="header-item ${requestScope.category.id eq category.id||fn:indexOf(requestScope.category.parentIds,menuCategoryId) ge 1?'active':''}"><a href="${category.url}" target="${category.target}"><span>${category.name}</span></a></li>
				    	</c:if></c:forEach>
					    <li id="siteSwitch" class="header-item dropdown">
	...
	<div class="container">
		...
		<footer>
			<c:choose>
				<c:when test="${cookie.theme.value == 'justice'}">
					<div class="footer_nav" style="text-align: center;"><a href="${ctx}/guestbook" target="_blank">公共留言</a> | <a href="${ctx}/map-${site.id}${fns:getUrlSuffix()}" target="_blank">站点地图</a> | <a href="mailto:thinkgem@163.com">技术支持</a> | <a href="${pageContext.request.contextPath}${fns:getAdminPath()}" target="_blank">后台管理</a></div>
					<div class="copyright" style="text-align: center;">${site.copyright}</div>
				</c:when>
				<c:otherwise>
